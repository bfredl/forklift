...
pub const Inst = struct {
    ...
    pub fn res_type(inst: Inst) ?ValType {
        return switch (inst.tag) {
            ...
            .copy => inst.mem_type(), // should be removed at this point
            .load => inst.mem_type(),
...
pub const callregs: [6]IPReg = .{ .rdi, .rsi, .rdx, .rcx, .r8, .r9 };
...
pub fn alloc_arg(self: *Self, inst: *Inst) !void {
    _ = self;
    if (inst.op1 >= callregs.len) return error.ARA;
    inst.mckind = .ipreg;
    inst.mcidx = callregs[inst.op1].id();
}
...
pub fn scan_alloc(self: *Self) !void {
    ...
    for (self.n.items, 0..) |*n, ni| {
        ...
        while (it.next()) |item| {
            ...
            // const ref = item.ref;
            if (i.tag == .arg) {
                if (i.op1 >= callregs.len) return error.FLIRError;
                i.mckind = .unallocated_ipreghint;
                i.mcidx = callregs[i.op1].id();
                try self.alloc_arg(i);
            }

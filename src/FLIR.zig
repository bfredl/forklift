...
pub fn call(self: *Self, node: u16, kind: defs.CallKind, num: u16, extra: u16, ret_type: ?defs.SpecType) !u16 {
    ...
    return if (ret_type) |typ|
        // TODO: this should just be a Empty padding. ABI copies should be generated by regalloc!
        self.addInst(node, .{ .tag = .copy, .spec = typ.into(), .op1 = c, .op2 = NoRef })
    else
        c; // TRICKY, still want to be able to track it for side effects and giggles???
}
...
const ABIType = union(enum) { ipreg: defs.IPReg, avxreg: u16 };
